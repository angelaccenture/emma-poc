 ...
 * Authored rows: [0] submit button label. (Any intro/heading text is authored
 * as default content OUTSIDE the form block, in the same section.)
 * Renders a <form> with First/Last/Email inputs and a submit button.
 * The fields mirror the source newsletter form; not wired to a backend (POC).
 */
export default function init(el) {
  const rows = [...el.querySelectorAll(':scope > div')];
  const buttonText = rows[0]?.textContent.trim() || 'Submit';
